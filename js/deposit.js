...
// РЕШЕНИЕ: Простейший подход - без глобальных флагов и сложной логики
...
export async function init() {
    ...
    if (depositInitialized) {
        console.log('Deposit уже инициализирован, пропускаем');
        return;
    }
    ...
    // ОЧЕНЬ ПРОСТАЯ установка обработчиков
...
function setupSimpleEventListeners() {
    ...
    // ГЛАВНАЯ КНОПКА ДЕПОЗИТА - САМАЯ ПРОСТАЯ РЕАЛИЗАЦИЯ
    ...
    if (copyBtn) {
        copyBtn.onclick = copyDepositAddress;
    }
}

// ОЧЕНЬ ПРОСТАЯ функция обработки клика
...
// Добавляем глобальную функцию для принудительного ресета
window.fixDepositButton = function() {
    ...
    console.log('Кнопка депозита переустановлена');
};
